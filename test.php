<?php

	require "db_connect.php";
	require "functions.php";
	require "personInfo.php";

?>

<html>
	<head>
	<link href="pasul_4.css" rel="stylesheet" type="text/css">
	<title>Пришла Весна! Pasul RU</title>
	</head>
	<body bgcolor = "FFFFF0">
	<a id="sus"></a>
		<div>
		<!-- Ссылки на другие страницы -->
			<table width = 100%>
				<tr>
					<td align = "right">
					<a href = "pasul_7.html"><font size = 4><img src = "md.png" width = 4%></font></a>
					</td>	
				</tr>
			</table>
		</div>
		
		<!-- Таблица навигации 2.0-->
		
		<table align = "center" width = 850px>
		<td>
			<div id="navBar1" >
			<ul>
			<li class="headerList1"><a href="#start"> Начало повести</a></li>
			<li class="headerList1"><span><a href="#red"> Редактировать</a></span>
			</li>
			<li class="headerList1"><span><a href="#tabel"> Таблица состояний</a></span>
			</li>
			<li class="headerList1"><span><a href="#"> Другие страницы</a></span>
			<ul>
			<li> <a href="pasul_1.html"> Шаг 1 </a></li>
			<li> <a href="pasul_2.html"> Шаг 2 </a></li>
			<li> <a href="pasul_3.html"> Шаг 3 </a></li>
			<li> <a href="pasul_4.html"> Шаг 4 </a></li>
			<li> <a href="pasul_5.html"> Шаг 5 </a></li>
			<li> <a href="pasul_6.html"> Шаг 6 </a></li>
			</ul>
			</li>
			<li><span><a href="https://www.povesti-pentru-copii.com/" id = "alttext"> 	Читайте другие книги</a></span>
			</li>
			</ul>
			</div>
		</td>
		</table>
		
		<br><br><br>
		
		
		<h1 align = "center">Пришла Весна!</h1>
		
		
		
		<!--Здесь содержится основной текст.-->
		<div class="border">
		<div class="border-top"></div>
		<div class="center"><font size = 4>
			<a id= "start"></a>
		
			<p>
			- Чипцирип! Чипцирип! Смотри, идет снег! Зима пришла!
			<br>
			 <?php echo"$vorobei";?> радостно <?php echo"$actionAutumnVorobei";?>. <?php echo"$dwlZaiats";?> - домик <?php echo"$zaiats";?>.<br>

        - Пришла зима ?! <?php echo"$zaiats";?> затрепетал ушами в окно. Какое чудо! Скоро наш лес будет одет в шубу цвета моей меха. Я знаю, почему зима выбирает этот цвет.
<br>
        - Зачем? - спрашивает <?php echo"$vorobei";?>.
</p><img id="im" src="iarna.jpg" width = 450px height = 550px><p>
        «Потому что зима так меня любит». Больше, чем у других животных.
<br>
        «У вас нет скромности», - сердито сказал <?php echo"$vorobei";?>. Ты хвастун. Другими словами, бабушка Зима была в белом халате еще до того, как вы, <?php echo"$zaiats";?>, появились на свете! И если хотите знать - Зима нас всех любит! Все!
<br>
        - Да! Да! К  <?php echo"$vorobei";?> присоединяется <?php echo"$belka";?>. Это так! Зима нас всех любит!
<br>
        «Хорошо, - сказал <?php echo"$zaiats";?>. Я ничего не говорю. И все же, шепчет он себе, он любит меня больше.
<br>
        "Боже!" Разве не идет снег ?! <?php echo"$medved";?> выходит из <?php echo"$dwlMedved";?>.
<br>
        - Идет снег, <?php echo"$medved";?>! Снег! радостно прыгают <?php echo"$vorobei";?>. Зима пришла!
<br>
        - Она пришла… Кто ее пригласил? Разве она не может остаться в своём доме? Хм… Я не понимаю, почему это меня усыпиляет… Когда там… Зима виноват. Морр, морр, морр! Придется поспать еще три месяца. Морр, морр, морр…
<br>

 
<?php echo"$medved";?> тоже удаляется в <?php echo"$dwlMedved";?>, но через несколько мгновений возвращается:
<br>
        - Ты слышишь ?! Он крикнул.
<br>
        "Кому ты звонишь, <?php echo"$medved";?>?" - спрашивает <?php echo"$vorobei";?>.
<br>
        «Ты», - отвечает <?php echo"$medved";?>. У меня есть запрос. Как только наступит весна, разбуди меня! Не дай мне спать.
<br>
        "Почему спишь так долго, <?php echo"$medved";?>?" Разве вы и остальные не можете спать только по ночам? Скоро придет <?php echo"$ded_moroz";?> и будет <?php echo"$actionWinterDedMoroz";?>. Это будет так красиво.
<br>
        - Э ... Я бы хотел ... Но не могу. Если я не сплю, я буду голодать. Моя бабушка прячет мою еду подо льдом, под снегом. Старуха меня не любит. Он меня не любит…
<br>

 
Когда сплю - мне снится, что лес полон клубники. А время идет… Не забудь меня разбудить… Пожалуйста!
<br>
«Мы разбудим тебя, <?php echo"$medved";?>, - обещает <?php echo"$vorobei";?> и затем улетает на ветви ёлки.<br>
Зима приносит в лес снег и много света. Жеванные и пушистые хлопья падают целыми днями. Все ждут Рождества. Приближается Рождество, и лес полон шума и веселья. <?php echo"$ded_moroz";?> <?php echo"$actionWinterDedMoroz";?>.
<br>
        «Этот <?php echo"$giftMedved";?>, - говорит он, роясь в мешке, - как вы думаете, для кого он?»
<br>
</p>
			
<img id ="im" src = "urs.jpg" width = 450px>

<p>
        - Для меня! - закричал <?php echo"$zaiats";?>.
<br>
        - Это мое! <?php echo"$belka";?> говорит старику. Кстати… я никогда не любила <?php echo"$giftMedved";?>. «Пусть его заберет <?php echo"$zaiats";?>, - решает она.
<br>
        «Этот <?php echo"$giftMedved";?>, - улыбается <?php echo"$ded_moroz";?>, - предназначен для <?php echo"$medved";?>. В такой день нельзя забывать никого. Ни один. Никто.
<br>
        - У <?php echo"$medved";?> есть друзья? - спрашивает <?php echo"$ded_moroz";?>.
<br>
        - Конечно! К <?php echo"$ded_moroz";?> подходят <?php echo"$vorobei";?>, <?php echo"$belka";?> и <?php echo"$zaiats";?>. Мы его друзья.
<br>
        «Молодцы», - посмотрел в глаза <?php echo"$ded_moroz";?>. Принеси ему <?php echo"$giftMedved";?>. Когда он проснется, скажите ему, что он от <?php echo"$ded_moroz";?> и ему повезло с такими друзьями.
<br>
И вас я без подарков не оставлю. <?php echo"$vorobei";?> получит <?php echo"$giftVorobei";?>, <?php echo"$belka";?> заслужила <?php echo"$giftBelka";?>,
а <?php echo"$zaiats";?> дарю <?php echo"$giftZaiats";?>.
<br>
После праздников лес выглядит безлюдным. Все отдыхают. <?php echo"$zaiats";?> редко <?php echo"$actionWinterZaiats";?>. <?php echo"$actionWinterVorobei";?> <?php echo"$vorobei";?>. <?php echo"$belka";?> грызет орехи в своей комнатке. 
Иногда веселая и в хорошем настроении она <?php echo"$actionWinterBelka";?>, наблюдая за тряской снега.
<br>
        «Я знаю, почему зима укрывает землю снежным покрывалом!» крикнула <?php echo"$belka";?>.
<br>
        - Зачем? - скучно спрашивает <?php echo"$vorobei";?>.
<br>
        "Чтобы согреть его!" Как <?php echo"$medved";?> - Земля спит три месяца. Там, под пуховым одеялом, лежит трава, лежит попкорн, который весной прорастет. Зимой боится, что мороз их всех заморозит.
<br>
        «Мороз не жалеет», - вздохнул <?php echo"$vorobei";?>. Когда наступит весна?
<br>
        - Не скоро… Зима только начинается. И <?php echo"$belka";?> входит в <?php echo"$dwlBelka";?>, чтобы грызть орехи.
<br>
Дни короткие, ночи длинные, погода тяжелая. Однако однажды Зима снимает свои белые простыни и уходит. Какая радость!
<br>
        - Чипцирип! Cipcirip! Весна здесь!
<br>
С приходом весны лес оживает. Работа кипит везде. <?php echo"$vorobei";?> спешит построить новый <?php echo"$dwlVorobei";?>. Соберите веточки, пряди сухой травы, склейте их и <?php echo"$dwlVorobei";?> готов.
<br>
        - Но где <?php echo"$medved";?>? Вспомнила <?php echo"$belka";?>.
<br>
<img id="im" src = "casa-urs.jpg" width = 450px >
        - Точняк! Я забыл! Встряхните <?php echo"$vorobei";?>.
<br>
        - Чипцирип! Cipcirip! Они бегут в <?php echo"$dwlMedved";?>  <?php echo"$medved";?>. Просыпайся, <?php echo"$medved";?>! Весна здесь!
<br>

В <?php echo"$dwlMedved";?> нет движения.<?php echo"$medved";?> <?php echo"$actionWinterMedved";?>
<br>
        «Он не просыпается», - вздохнул <?php echo"$vorobei";?>.
<br>
        «Что-то могло случиться с ним», - сказал другой.
<br>
        «Мы должны пойти и посмотреть», - предлагает <?php echo"$belka";?>.
<br>
Воздух в <?php echo"$dwlMedved";?>, где спит <?php echo"$medved";?>, такой душный, такой твердый, что все трое теряют сознание.
<br>
        - Давайте окна откроем! - крикнула <?php echo"$belka";?>. Проветриваем, впускаем свет!
<br>
        «Слава богу, он жив». Какой здесь беспорядок
<br>
И <?php echo"$vorobei";?> вместе с <?php echo"$belka";?>, к которым присоединяется <?php echo"$zaiats";?>, сразу же приступают к работе. За короткое время <?php echo"$dwlMedved";?> <?php echo"$medved";?> неузнаваем.  <?php echo"$belka";?> <?php echo"$actionSpringBelka";?>. 
Старые занавески заменяются новыми, розовыми, с разноцветными пчелами и бабочками. Пол вымыт и застелен мягкими ковриками. У стены длинный стул, покрытый полосой всех цветов. Больше нечего делать. 
Друзья садятся на стул и ждут, пока <?php echo"$medved";?> проснется.
<br>
        - Просыпайся, <?php echo"$medved";?>! Просыпайся! <?php echo"$vorobei";?> теряет самообладание. Весна здесь! Нет времени спать!
<br>
        - Вы знаете что-нибудь? Говорит <?php echo"$vorobei";?>. Я собираюсь <?php echo"$actionSpringVorobei";?>. Глазки откроет сразу.
<br>
        «Привет», - улыбается <?php echo"$zaiats";?> из-под усов. Иди ... Лапа <?php echo"$medved";?> и думает, что у него есть ещё время поспать.
<br>
		- Затем? Что делать? Если подождать, пока он проснется добровольно - зимой он нас ловит.
		<img id="im" src = "primavara.jpg" width = 450px  >
<br>
        «Не сердись», - советует <?php echo"$belka";?>. Теперь разбудим его. Принеси <?php echo"$giftMedved";?> от <?php echo"$ded_moroz";?> Ставьте самовар. Аромат <?php echo"$giftMedved";?> такой сильный - <?php echo"$medved";?> не устоит.
<br>
Чайный пар наполняет всю <?php echo"$dwlMedved";?>.

<br>
        «Какой приятный запах», - <?php echo"$medved";?> открыл один глаз. Пахнет весной. Ой! Не так ли? Он пришел? Он испуганно выпрыгивает постели.
<br>
        - Это здесь, это здесь! Если вы еще спите, весна уходит незаметно, - шутит <?php echo"$zaiats";?>.
<br>
<?php echo"$medved";?> <?php echo"$actionSpringMedved";?>, выходит из <?php echo"$dwlMedved";?>. Он вытягивает лапы, дышит всеми легкими. Лес почти зеленый. Везде цветы, жужжание пчел, щебетание птиц. Небо голубое, солнышко счастливое.
<br>
		- Какая чудесная весна, <?php echo"$medved";?> возвращается к своим друзьям. По этому поводу неплохо было бы повеселиться! Я голоден как волк! Затем, заметив <?php echo"$giftMedved";?>, он подносит его ко рту и не 
		останавливается, пока не доходит до дна. Незаметно крадутся <?php echo"$vorobei";?>, <?php echo"$zaiats";?> и <?php echo"$belka";?>.
<br>
        - Фу… Я его разбудил, <?php echo"$vorobei";?> веселится. А теперь за работу! Не время веселья! Весенний день год кормит!
		<br>
			</p>
			<br>
			</div>
		<div class="border-bottom"></div>
	</div>
		<br>
		<a id= "red"></a>
		<br>

		
	<table align="center" cellspacing="5.5" width = 60% border="2px"> 
		<th>Добавить персонажа</th>
		<th>Изменить подарок <br> полученный <?php echo"$zaiats";?></th>
		<th>Удалить персонажа</th>
		
		<tr>
			<!--Добавление персонажа-->
			<td>
				<form align = "center" method="post" action="createPerson.php">
				<br>
				<p>Имя персонажа	<br>
				<input type="text" name="person_name" maxlength="20">
				</p>
				<p>Действие персонажа	<br>
				<input type="text" name="activity" maxlength="100" >
				</p>
				<p>Время года (когда <br> происходит действие) <br>
				<input type="text" name="season" maxlength="20" >
				</p>
				<p>Подарок полученный<br> персонажем <br>
				<input type="text" name="gift" maxlength="50">
				</p>
				<p>
				<p>Жилище персонажа <br>
				<input type="text" name="dwelling" maxlength="50">
				</p>
				<br>
				<button type="submit">Добавить</button>
				</p>
				</form>
			</td>

			<!--Изменение персонажа-->
			<td>
			<form align = "center" method="post" action="updatePerson.php">
			<br>
				
				<p>Подарок полученный<br> <?php echo"$zaiats";?> <br><br>
				<input type="text" name="gift" maxlength="50">
				</p>
				
				<br>
				<button type="submit">Изменить</button>
				</p>
				</form>
			</td>

			<!--Удаление персонажа-->
			<td>
			<form align = "center" method="post" action="deletePerson.php">
			<br>
				<p>Действие персонажа<br></p>

				<select name="del_activity">
				<?php 
				$activityToDelete = getActivity();

				foreach($activityToDelete as $a)
				echo "<option value=".$a['id_action'].">".$a['name_action']."</option>";
				?>
				</select>
			
				<br><br>
				<button type="submit">Удалить</button>
				</p>
				</form>
			</td>
		</tr>
		</table>



		<br><br>
		<a id= "tabel"></a>
		<!-- Таблица состояний -->
		<div align = "center" id="tab">
			<table class ="tt"border = 2px cellspacing="5.5" width = 60% height = 900px>
			<tr>
				<td colspan = "3" >
				<p align = "center">Таблица состояния персонажей</p>
				</td>
			</tr>
			<th>
			<p>Персонаж</p>
			</th>
			
			<th>
			<p>Состояние</p>
			</th>
			<th>
			<p>Время года</p>
			</th>
			<?php
				foreach($activity as $act) {
						
					if($act['name_season'] == 'winter')
					{
						echo "<tr bgcolor = '00BFFF'>";		//hashmap
					}
					else if($act['name_season'] == 'spring')
					{
						echo "<tr bgcolor = '7FFF00'>";
					}
					else if($act['name_season'] == 'autumn')
					{
						echo "<tr bgcolor = 'fbec5d'>";
					}
					else if($act['name_season'] == 'summer')
					{
						echo "<tr bgcolor = 'ff033e'>";
					}
					else echo "<tr>";
					echo "
						<td>	<p align = 'center'>
						".
						$act['name_person']."
						</p>	</td>

						<td>	<p>
						".
						$act['name_action']."
						</p>	</td>

						<td>	<p>
						".
						$act['name_season']."
						</p>	</td>
					</tr>";
				}
			?>
			
			
			</table>
		</div>
		
		<a href = "#sus"><img id="oksus" src = "sus.png" width = 4% align = "right"></a>
	</body>
</html>